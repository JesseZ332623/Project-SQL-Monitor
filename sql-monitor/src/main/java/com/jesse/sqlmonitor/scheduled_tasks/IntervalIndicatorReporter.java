...
public class IntervalIndicatorReporter implements DisposableBean
{
    ...
    /**
     * 定期以邮件形式向运维报告数据库服务器相关指标数据。
     * 条目如下：
     *
     * <li>
     *     <ul>距离上一次报告时数据增长的数量</ul>
     *     <ul>数据库平均 QPS</ul>
     *     <ul>数据库 QPS 极值</ul>
     *     <ul>数据库 QPS 中位数</ul>
     *     <ul>数据库 QPS 标准差</ul>
    ...
    sendIntervalIndicatorReport()
    {
        ...
        {
            log.warn("The task sendIntervalIndicatorReport() already executing, skip...");
            return Mono.empty();
        }
    ...
    makeIndicatorReport()
    {
        ...
        Mono.zip(
             this.monitorLogRepository
                 .getIndicatorIncrementByDuration(serverIp, Duration.ofHours(1L)),
             this.monitorLogRepository
    ...
    makeIndicatorReportEmail(@NotNull IndicatorReport report)
    {
        ...
        EmailContent.fromJustText(
            ...
            "【数据库指标监视器】例行数据库指标报告",
            """
            截止 %s，数据库（IP 地址：%s，端口：%s）共增长 %d 条指标数据，
            当前 QPS 平均值 = %f
                 QPS 中位数 = %f
                 最大 QPS = %f，最小 QPS = %f，
                 QPS 标准差 = %f，负载均衡律 = %f
